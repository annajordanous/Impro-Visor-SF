...
public class Phrase implements JMC, Cloneable, Serializable{
    ...
    * Return the phrases endTime
	...
	public double getEndTime(){	
		...
		Enumeration e = this.noteList.elements();
		while(e.hasMoreElements()){
			Note nextNote = (Note)e.nextElement();
    ...
    * Determine the pan position for all notes in this phrase. 
	...
	public void setPan(double pan){
		this.pan = pan;
		Enumeration e = noteList.elements();
		while(e.hasMoreElements()){
			Note note = (Note) e.nextElement();
			note.setPan(pan);
	    }
    ...
    * Return the tempo in beats per minute for this phrase
	...
	public Phrase copy(){
		...
		Enumeration e = this.noteList.elements();
		while(e.hasMoreElements()){
			phr.addNote( ((Note) e.nextElement()).copy() );
		}
		return phr;
	}
	...
	public Phrase copy(int highestPitch, int lowestPitch){
		...
		Enumeration e = this.noteList.elements();
		while(e.hasMoreElements()){
            Note n = ((Note)e.nextElement()).copy();
	...
	public String toString(){
		...
		Enumeration e = getNoteList().elements();
		int counter = 0; 
		while(e.hasMoreElements()){
			Note note = (Note) e.nextElement();
