...
public final class MidiParser implements JMC{
	...
	public static void SMFToScore(Score score, SMF smf){
		...
		Enumeration e = smf.getTrackList().elements();
		//Go through tracks
	...
	public static void scoreToSMF(Score score, SMF smf){
		...
		//---------------------------------------------------
		int partCount = 0;
		Enumeration e = score.getPartList().elements();
		while(e.hasMoreElements()){
			Track smfTrack = new Track();
			Part inst = (Part) e.nextElement();
			...
			while(partEnum.hasMoreElements()) {
				...
				while(phraseEnum.hasMoreElements()){
					...
					if(note.getPan() != pan) {
						pan = note.getPan();
						midiEvents.put(new Double(startTime + offsetValue),new CChange((short)10,(short)(pan * 127), (short)inst.getChannel(),0));
					}
