...
//------------------------------------------
/**
 * Create a new empty SMF with default filetype and ppqn settings SMF objects
 * are useful for retrieving statistical information about a MIDI file which has
...
/**
 * Create a new empty SMF. Allows user to set filetype and ppqn settings SMF
 * Objects are useful for retrieving statistical information about a MIDI file
 * which has just been read or written.
 */
...
//------------------------------------------
/**
 * Read from a standard MIDI file @params InputStream - the datasource to read
 * from @params Score score - the score to place jMusic data translation into
 *
...
public void read(InputStream is)
        throws IOException
  {
    ...
    if( dis.readInt() != 0x4D546864 )
      {//Check for MIDI track validity
        throw new IOException("This is NOT a MIDI file !!!");
      }
    else
      {//If MIDI file passes skip length bytes
...
public void write(OutputStream os)
        throws IOException
  {
    ...
    //write all tracks
    Enumeration e = trackList.elements();
    while( e.hasMoreElements() )
      {
        Track smfTrack = (Track) e.nextElement();
        ...
        try
          {
            ...
            if( status >= 0xFF )
              ...
              { //System Exclusive --- NOT SUPPORTED
                System.out.println("note: SysEX in MIDI file is not supported");
                eventLength = MidiUtil.readVarLength(dis);
              }
            else if( status >= 0x80 )
              { //MIDI voice event 
...
private void writeTrackChunk(DataOutputStream odos, Track track)
        throws IOException
  {
    ...
    Enumeration e = track.getEvtList().elements();
    e = track.getEvtList().elements();
