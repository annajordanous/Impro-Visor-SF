...
/**
 * 
 * @author Brandy McMenamy and James Thomas Herrold
 * June 2007
 *
 * BassChronotonic:
 * input:  an ArrayList<String> where each String is a bass-line style-
 * specification of pitches and rhythms (like Impro-Visor's bass rules)
 * 
 * Creates a two dimensional matrix that describes the chronotonic measurement 
 * between each rule.  The indices of the matrix directly correlate to the indices
 * of the rules in the original ArrayList and row k and col k refer to the same rule.
 * 
 * Only rules of the same length can have a chronotonic value.  The default value -1 is 
 * used for two rules of unequal length
 */
...
public class BassChronotonic {
	 ...
	 * specification of pitches and rhythms
         * @precondition: MIDIBeast.originalBassRules does not contain any mal-formed rules
	 */
	public BassChronotonic() {
             /*orignalBassRules, which is processed earlier by BassPatternGenerator, should be
              formatted correctly.  This protection is added just in case something
	 ...
	 * @param r - the chronotonic values for a single rule (a row or column in chronoValues)
	 */
