...
public class CreateGrammar implements imp.Constants {
    ...
    /* Based on hard-coded filename, this can't be current.
    
    //writeClosenessForR, graphCloseness, and arrangeClosenessValues are experimental code 
      
    private static void writeClosenessForR(Vector<Double> closeNess) {
        try {
            BufferedWriter out = new BufferedWriter(new FileWriter
                    ("/Users/Jon/Desktop/R Working Directory/closenessGenerated.txt", true));          
                out.write("Distance\n");
                for(int i = 0; i < closeNess.size(); i++) {
                    double d = closeNess.get(i);
    ...
    private static void arrangeClosenessValues(Vector<Double> closeNess) {
        Collections.sort(closeNess);
        int count = 1;
        for(int i = 0; i < closeNess.size()-1; i++) {
            if(closeNess.get(i).equals(closeNess.get(i+1))) {
                count++;
            }
            else {
                System.out.println(closeNess.get(i) + " occured " + count + " times. That's " + 
                        (int)((double)count / (double)closeNess.size() * 100) + " percent."  );
                count = 1;
            }
        }
    }

    private static void graphCloseness(Vector<DataPoint> dataPoints) {
        //get all chorus numbers
        Vector<Integer> chorusNumbers = new Vector<Integer>();
        for(int i = 0; i < dataPoints.size(); i++) {
            Integer cNum = new Integer(dataPoints.get(i).getChorusNumber());
            if(!chorusNumbers.contains(cNum))
                chorusNumbers.add(cNum);
        }
        
        for(int i = 0; i < chorusNumbers.size(); i++) {
            int cNum = chorusNumbers.get(i);
            int count = 0;
                String rep = "";
            for(int j = 0; j < dataPoints.size(); j++) {
                if(dataPoints.get(j).getChorusNumber() == cNum && count <16 ) {
                    count++;
                    if(dataPoints.get(j).getSimilarityToHead() > 0.3)
                        rep = rep.concat("X ");
                    else rep = rep.concat("O ");
                    if (count % 4 == 0) rep = rep.concat("\n");
                }
            }
            System.out.println("Chorus " + cNum + ":\n" + rep);
        }
    }
    ...
    */ 
    ...
    // FIRST ATTEMPT.... DEALS WITH DATA EXTRACTED BY GETALLCONTOURS IN NOTATE
    /* Hopefully no longer used, as filename is bogus.       
      
    public static void getContourData() {
        Vector<float[]> phraseData = new Vector<float[]>();

        int maxPhraseDuration = 0;


        try {
            FileInputStream f_in = new FileInputStream("C:/Documents and Settings/keller/Desktop/Improvisor339/Contours/Contours.data");
            ...
            if (obj instanceof Vector) {
                Vector<Vector<Vector<int[]>>> phrases = (Vector<Vector<Vector<int[]>>>) obj;

                //get max phrase duration and max number of notes in phrase
                int[] maximums = getMaximumNotesAndDuration(phrases);
                int[] notesPerPhrase = new int[maximums[0] + 1];
                int[] phraseDurations = new int[maximums[1] + 1];

                int bigCount = 0;
                int totalSegs = 0;
                boolean stupidPhrase = false;
                //loop through songs
                for (int i = 0; i < phrases.size(); i++) {
                    //System.out.println("New Song!");
                    //loop through phrases
                    for (int j = 0; j < phrases.get(i).size(); j++) {
                        int phraseDuration = 0;
                        int notesInPhrase = 0;
                        float sumOfSlopes = 0;
                        //System.out.println("New phrase!");
                        //loop through segments
                        int k;
                        for (k = 0; k < phrases.get(i).get(j).size(); k++) {
                            totalSegs++;
                            int[] segment = (int[]) phrases.get(i).get(j).get(k);
                            if (segment[4] > 12 || segment[3] < -12) {
                                stupidPhrase = true;
                            }
                            phraseDuration += segment[1];
                            //increment counters for notesPerPhrase distribution
                            if (k == phrases.get(i).get(j).size() - 1) {
                                notesPerPhrase[segment[2]]++;
                                notesInPhrase = segment[2];
                            }

                            sumOfSlopes += Math.abs(segment[4]);


                        //System.out.println("Pitch: " + segment[0] + " Duration: " + segment[1] +
                        //        " Number of notes: " + segment[2] + " Min interval: " + segment[3] + " Max Interval: " + segment[4]);
                        }

                        if (!stupidPhrase) {
                            phraseDurations[phraseDuration]++;
                            float[] tempPhraseData = new float[6];
                            tempPhraseData[0] = notesInPhrase;
                            tempPhraseData[1] = phraseDuration;
                            tempPhraseData[2] = (float) phraseDuration / (float) notesInPhrase; //average note length
                            tempPhraseData[3] = k - 1; //number of segments/order
                            tempPhraseData[4] = (phraseDuration / (k - 1));  //average segment length
                            tempPhraseData[5] = sumOfSlopes / (k - 1);  //average slope of segment
                            phraseData.add(tempPhraseData);
                        }
                        stupidPhrase = false;
                    }
                }

                float[] averages = getAverages(phraseData);
                phraseData = normalizeData(phraseData, averages);
                Cluster[] clusters = getClusters(phraseData, averages);
                Vector<Vector<int[]>> allPhrases = getAllPhrases(phrases);
                int[] clusterReps = getRepsFromClusters(clusters);
                writeClusterRepsToFile(allPhrases, clusterReps);
            //int[] orderedClusters = getClusterOrder(clusters, phraseData);
            //for (int i = 0; i < orderedClusters.length; i++) {
            //    System.out.println(i + 1 + ": Cluster " + orderedClusters[i]);
            //}
    ...
    //writes a vector of phrases, each of which represents a cluster, to a file
    private static void writeClusterRepsToFile(Vector<Vector<int[]>> allPhrases, int[] clusterReps) {
        File output = new File("C:/Documents and Settings/keller/Desktop/Improvisor339/Contours/ClusterReps.data");
        //create the vector
        Vector<Vector<int[]>> reps = new Vector<Vector<int[]>>();
        for (int i = 0; i < clusterReps.length; i++) {
            reps.add(allPhrases.get(clusterReps[i]));
        }

        for (int i = 0; i < reps.size(); i++) {
            //System.out.println("Cluster " + i + ": ");
            for (int j = 0; j < reps.get(i).size(); j++) {
                int[] segment = reps.get(i).get(j);
            //System.out.println("Pitch: " + segment[0] + " Duration: " + segment[1] +
            //        " Number of notes: " + segment[2] + " Min interval: " + segment[3] + " Max Interval: " + segment[4]);
    ...
    public static Vector<Vector<int[]>> getAllPhrases(Vector<Vector<Vector<int[]>>> phrases) {
        boolean stupidPhrase = false; //throw out phrase if it is stupid
        Vector<Vector<int[]>> allPhrases = new Vector<Vector<int[]>>();
        //loop through all songs
        for (int i = 0; i < phrases.size(); i++) {
            //loop through phrases
            for (int j = 0; j < phrases.get(i).size(); j++) {
                for (int k = 0; k < phrases.get(i).get(j).size(); k++) {
                    int[] segment = (int[]) phrases.get(i).get(j).get(k);
                    if (segment[4] > 12 || segment[3] < -12) {
                        stupidPhrase = true;
                    }
                ...
                if (!stupidPhrase) {
                    allPhrases.add(phrases.get(i).get(j));
                }
                stupidPhrase = false;
            }
        }
    ...
    // Takes an array of cluster
    // gets a representative from each cluster by taking the element closest
    // to the average of all elements in the cluster
    
    public static int[] getRepsFromClusters(Cluster[] clusters) {
        double[] averages;
        int[] representatives = new int[clusters.length];
        for (int i = 0; i < clusters.length; i++) {
            averages = new double[4];
            Cluster tempCluster = clusters[i];
            for (int j = 0; j < clusters[i].getNumDataPoints(); j++) {
                DataPoint tempPoint = tempCluster.getDataPoint(j);
                averages[0] += tempPoint.getW();
                averages[1] += tempPoint.getX();
                averages[2] += tempPoint.getY();
                averages[3] += tempPoint.getZ();
            }
            averages[0] /= clusters.length;
            averages[1] /= clusters.length;
            averages[2] /= clusters.length;
            averages[3] /= clusters.length;
            DataPoint avgPoint = new DataPoint(averages[0], averages[1], averages[2], averages[3], "averages");
            double minDistance = 9999999;
            String clusterName = "";
            for (int k = 0; k < tempCluster.getNumDataPoints(); k++) {
                double distance = avgPoint.calcEuclideanDistance(tempCluster.getDataPoint(k));
                if (distance < minDistance) {
                    minDistance = distance;
                    clusterName = tempCluster.getDataPoint(k).getObjName();
                }
            }

            representatives[i] = Integer.parseInt(clusterName);
        }
        return representatives;
    }

    public static Vector<float[]> normalizeData(Vector<float[]> phraseData, float[] averages) {
        for (int i = 0; i < phraseData.size(); i++) {
            float[] phrase = phraseData.get(i);
            phrase[1] /= (averages[0] / 100);
            phrase[2] /= (averages[1] / 100);
            phrase[4] /= (averages[2] / 100);
            phrase[5] /= (averages[3] / 100);
            phraseData.setElementAt(phrase, i);
        }
        return phraseData;
    }

    //takes a set of analyzed data and returns averages
    public static float[] getAverages(Vector<float[]> phraseData) {
        float totalDuration = 0;
        float totalNoteLength = 0;
        float totalSegmentLength = 0;
        float totalMaxSlope = 0;
        for (int i = 0; i < phraseData.size(); i++) {
            float[] phrase = phraseData.get(i);
            totalDuration += phrase[1];
            totalNoteLength += phrase[2];
            totalSegmentLength += phrase[4];
            totalMaxSlope += phrase[5];
        }

        System.out.println("Average duration: " + totalDuration / phraseData.size() / 120);
        System.out.println("Average Note length: " + totalNoteLength / phraseData.size() / 120);
        System.out.println("Average Segment length: " + totalSegmentLength / phraseData.size() / 120);
        System.out.println("Average Max Slope: " + totalMaxSlope / phraseData.size());

        float[] averages = new float[4];

        averages[0] = totalDuration / phraseData.size();
        averages[1] = totalNoteLength / phraseData.size();
        averages[2] = totalSegmentLength / phraseData.size();
        averages[3] = totalMaxSlope / phraseData.size();
    ...
    public static int[] getClusterOrder(Cluster[] clusters, Vector<float[]> phraseData) {
    int[] order = new int[phraseData.size()];
    for (int i = 0; i < phraseData.size(); i++) {
    for (int j = 0; j < clusters.length; j++) {
    for (int k = 0; k < clusters[j].getNumDataPoints(); k++) {
    if (clusters[j].getDataPoint(k).getObjName().equals(Integer.toString(i))) {
    order[i] = j;
    k = clusters[j].getNumDataPoints();
    j = clusters.length - 1;
    }
    }
    }
    ...
    public static Cluster[] getClusters(Vector<float[]> phraseData, float[] averages) {
        Vector dataPoints = new Vector();
        for (int i = 0; i < phraseData.size(); i++) {
            //temporarily added 0 and "" for new datapoint
            dataPoints.add(new DataPoint(phraseData.get(i)[1], phraseData.get(i)[2], phraseData.get(i)[4], phraseData.get(i)[5], Integer.toString(i)));
        }
        JCA jca = new JCA(100, dataPoints.size(), dataPoints);
        jca.startAnalysis();
        ...
        for (int i = 0; i < clusters.length; i++) {
            ...
            for (int j = 0; j < tempCluster.getNumDataPoints(); j++) {
                DataPoint dpTemp = tempCluster.getDataPoint(j);
            //if((dpTemp.getX()/120) > max) max = dpTemp.getX()/120;
            //System.out.println("[" + dpTemp.getObjName() + "] Duration: " + dpTemp.getW() * (averages[0] / 100) / 120 + ", average note length: " + dpTemp.getX() * (averages[1] / 100) / 120 + ", average segment length: " + dpTemp.getY() * (averages[2] / 100) / 120 + ", max slope: " + dpTemp.getZ() * (averages[3] / 100));
            }
    ...
    public static void printPhraseData(Vector<float[]> phraseData) {
        System.out.println(phraseData.size() + " total phrases.");
        for (int i = 0; i < phraseData.size(); i++) {
            System.out.println("Number of notes: " + phraseData.get(i)[0] + "  Duration:  " + phraseData.get(i)[1] + "  Average note length:  " + phraseData.get(i)[2] + "  Order: " + phraseData.get(i)[3] + "  Average length of segment  " + phraseData.get(i)[4] + "  Average slope of segment:  " + phraseData.get(i)[5]);
        }

    }

    public static void printPhraseDurationDistribution(int[] phraseDurations) {
        int indexCounter = 0;
        while (indexCounter < phraseDurations.length) {
            int counter = 0;
            String indices = "";
            String vals = "";
            while (counter < 10 && indexCounter < phraseDurations.length) {
                if (phraseDurations[indexCounter] != 0) {
                    indices = indices.concat(Integer.toString(indexCounter)).concat(" ");
                    vals = vals.concat(Integer.toString(phraseDurations[indexCounter])).concat(" ");
                    counter++;
                }
                indexCounter++;
            }
            System.out.println("Phrase Duration: " + indices);
            System.out.println("Occurences:      " + vals + "\n");
        }
    }

    public static void printNotePerPhraseDistribution(int[] notesPerPhrase) {
        int indexCounter = 0;
        while (indexCounter < notesPerPhrase.length) {
            int counter = 0;
            String indices = "";
            String vals = "";
            while (counter < 10 && indexCounter < notesPerPhrase.length) {
                indices = indices.concat(Integer.toString(indexCounter)).concat(" ");
                vals = vals.concat(Integer.toString(notesPerPhrase[indexCounter])).concat(" ");
                counter++;
                indexCounter++;
            }
            System.out.println("Number of notes: " + indices);
            System.out.println("Occurences:      " + vals + "\n");
        }
    }

    public static int[] getMaximumNotesAndDuration(Vector<Vector<Vector<int[]>>> phrases) {
        int maxPhraseDuration = 0;
        int maxNotesPerPhrase = 0;
        //get max phrase duration and max number of notes in phrase
        for (int i = 0; i < phrases.size(); i++) {
            for (int j = 0; j < phrases.get(i).size(); j++) {
                int phraseDuration = 0;
                for (int k = 0; k < phrases.get(i).get(j).size(); k++) {
                    int[] segment = (int[]) phrases.get(i).get(j).get(k);
                    if (k == phrases.get(i).get(j).size() - 1) {
                        if (segment[2] > maxNotesPerPhrase) {
                            maxNotesPerPhrase = segment[2];
                        }
                    }
                    phraseDuration += segment[1];
                }
                if (phraseDuration > maxPhraseDuration) {
                    maxPhraseDuration = phraseDuration;
                }
            }
        }
        int[] maximums = {maxNotesPerPhrase, maxPhraseDuration};
        return maximums;
    }
    ...
    */
    ...
    /* More experimental code here
    
     *     public static void printClusterHierarchy(ClusterHierarchy h) {
        Vector<Node> bottomLevel = h.getNodesAtLowestLevel();
            while(bottomLevel.size() >= 1) {
                
            
                int numCs = bottomLevel.size();
                int avSize = h.getDataPoints().size() / numCs;
                
                int minClusterSize = bottomLevel.get(0).getSize();
                int maxClusterSize = bottomLevel.get(0).getSize();
                
            //Cluster[] clusters = new Cluster[bottomLevel.size()];
            //int c = 0;
            for(int i = 0; i < bottomLevel.size(); i++) {
                //clusters[i] = bottomLevel.get(i).getCluster();
                //Vector<DataPoint> dps = bottomLevel.get(i).getDataPoints();
                //c += dps.size();
                //System.out.println(bottomLevel.get(i).getDataPoints().size() + " Reps.");
                int cSize = bottomLevel.get(i).getSize();
                if (cSize < minClusterSize) minClusterSize = cSize;
                if (cSize > maxClusterSize) maxClusterSize = cSize;
            }
                System.out.println("Num Clusters: " + numCs + " Average Size: " + avSize
                        + " Min Cluster Size: " + minClusterSize 
                        + " Max Cluster Size: " + maxClusterSize);
            //System.out.println(bottomLevel.size() + " total clusters.");
            //System.out.println("Total reps: " + c);
                if(bottomLevel.size() == 1) break;
                for(int i = 0; i < bottomLevel.size(); i++) {
                    Node n = bottomLevel.get(i);
                    Vector<DataPoint> v = n.getDataPoints();
                    for(int j = 0; j < v.size(); j++) {
                        System.out.println("Cluster at node " + i + ": " + v.get(j).getObjData());
                    }
                }
                bottomLevel = h.getNodesAtNextLevelUp(bottomLevel);
            }
    }
    */
