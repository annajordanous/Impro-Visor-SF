...
Polylist baseList = null;
Polylist ruleList = null;
ArrayList<Polylist> params = null;
 ...
 * terminalString, which is behaving as a queue.
 */
...
/**
 * Get the duration of various terminals. This is used in calculating chordSlot,
 * for example.
 */
...
/**
 * At the outset of loading grammar, triage the rules into these categories BASE
 * regular rules terminals adding each one to an appropriate list or structure.
 */
void triageRules()
  {
    PolylistBuffer baseListBuffer = new PolylistBuffer();
    PolylistBuffer ruleListBuffer = new PolylistBuffer();
    params = new ArrayList<Polylist>();
    ...
    while( search.nonEmpty() )
      {
        ...
        if( type.equals(BASE) && next.length() == 4 )
          {
            baseListBuffer.append(next);
          }
        else if( type.equals(RULE) && next.length() == 4 )
          {
            ruleListBuffer.append(next);
          }
          ...
          {
            Polylist terms = next.rest();
            while( terms.nonEmpty() )
              {
                terminals.add((String) terms.first());
                terms = terms.rest();
              }
          }
        search = search.rest();
      }
    baseList = baseListBuffer.toPolylist();
    ruleList = ruleListBuffer.toPolylist();
  }
...
/**
 * Inner class representation of rhs right-hand sides (RHS) with weights, for
