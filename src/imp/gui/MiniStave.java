 ...
 *
 *
 * @author Aaron Wolin & Stephen Jones (revised from code by Andrew Brown &
 *         Adam Kirby)
 * @version 1.0, 28th June 2005
 */
public class MiniStave
        extends JPanel
   ...
   */
  private MiniStaveActionHandler handleActions;
   ...
   */
  private Image trebleClef,  bassClef,  dot,  demisemiquaverRest,  semiquaverRest,  quaverRest,  crotchetRest,  minimRest,  semibreveRest,  one,  two,  three,  four,  five,  six,  seven,  eight,  nine,  zero,  delete,  tieOver,  tieUnder,  tupletBracket,  beatBracket;

  private Image[] crotchetUp,  crotchetDown,  quaverDown,  quaverUp,  semiquaverDown,  semiquaverUp,  demisemiquaverUp,  demisemiquaverDown,  minimDown,  minimUp,  semibreve,  sharp,  flat,  natural;

  /**
   * The notation window that Stave is in
   */
  protected MiniNotate notate;
  ...
  private int[] notePosOffset = {24, 24, 20, 20, 16, 12, 12, 8, 8, 4, 4, 0};

  private int[] sharps = {77, 72, 79, 74, 69, 76, 71};

  private int[] flats = {71, 76, 69, 74, 67, 72, 65};

  private int[] lineNotes = {0, 1, 4, 7, 8, 11, 14, 15, 17, 18, 21, 22};
 ...
 /**
  ...
  //private Color selectionColor = new Color(248, 253, 255, 80);
  ...
  // Triangle selectors
  Polygon selectionHandle[] = {new Polygon(new int[] {0, 8, 0}, new int[] {0, 0,
                                                                           8
                       }, 3), // Left top
                               new Polygon(new int[] {0, 0, 8},
                               new int[] {0, -8, 0}, 3), // Left bottom
                               new Polygon(new int[] {0, -8, 0},
                               new int[] {0, 0, 8}, 3), // Right top
                               new Polygon(new int[] {0, -8, 0},
                               new int[] {0, 0, -8}, 3) // Right bottom
  };
   ...
   */
  public MiniStave(MiniNotate notate)
    {
   ...
   * @param notate        the notation window for the Stave
   */
  public MiniStave(StaveType type, MiniNotate notate, String staveTitle)
    {
   ...
   * @param notate        the notation window for the Stave
   */
  public MiniStave(MelodyPart part, MiniNotate notate, String staveTitle)
    {
   ...
   * @param notate        the notation window for the Stave
   */
  public MiniStave(MelodyPart part, StaveType type, MiniNotate notate,
                String staveTitle)
    {
    super();
    this.origPart = part;
    ...
    handleActions = new MiniStaveActionHandler(this, notate);
...
//        selectionControls.setLocation(100,100);
//        selectionControls.setVisible(false);
//        this.add(selectionControls);
    ...
    partTitleEditor = new EntryPopup("part title", partTitleFont)
      ...
      public String staveToText()
        {
        return MiniStave.this.getPartTitle();
        }
    ...
    showTitleEditor = new EntryPopup("show", partTitleFont)
      ...
      public void textToStave(String text)
        {
        MiniStave.this.notate.getScore().setShowTitle(text);
        }

      public String staveToText()
        {
        return MiniStave.this.notate.getScore().getShowTitle();
        }
    ...
    Images images = ToolkitImages.getInstance();
    ...
    tupletBracket = images.getTupletBracket();
    beatBracket = images.getBeatBracket();
  ...
  public void changeType(StaveType type)
    {
    if( type == StaveType.GRAND || type == StaveType.TREBLE || type == StaveType.BASS )
      {
      this.type = type;
      }
    else if( type == StaveType.AUTO )
      {
      if( origPart.getHighestPitch() < A6 && origPart.getLowestPitch() > notate.score.getBreakpoint() )
        {
        this.type = StaveType.TREBLE;
        }
      else if( origPart.getHighestPitch() <= notate.score.getBreakpoint() && origPart.getLowestPitch() > B1 )
        {
      ...
      {
      System.out.println("Unknown stave type passed: " +
   ...
   */
  public MiniStaveActionHandler getActionHandler()
    {
...
/**
  ...
  // FIX: Is it a good idea to use this, as it discards rests!
  ...
  public void setSubDivs(int beat, int subdivs)
    {
    int slotsPerDivision = beatValue / subdivs;
    if( subdivs == 0 || slotsPerDivision  == 0 )
      {
      return;
    ...
    for( int i = beatValue * beat; i < cstrLines.length && i < beatValue * (beat + 1); i++ )
      {
      if( i % slotsPerDivision == 0 )
        {
        cstrLines[i] = new CstrLine(slotsPerDivision);
        }
      else
        {
        cstrLines[i] = null;
  ...
  /**
   * Flag for if a Note is part of a n-tuplet, such as a triplet
   */
  private boolean nTuplet = false;

  /**
   * Int value corresponding to the type of tuplet the note is in
   */
  private int nTupletValue = 0;
  ...
  /**
   * Flag for if the Unit is a Note
   */
  ...
  public class EntryPopup
    ...
    public void repaintBounds()
      {
      MiniStave.this.repaint(0, 0, panelWidth, headSpace);
      }
    ...
    private void updatePosition()
      {
      int x = MiniStave.this.notate.getX() + bounds.x;
      int y = MiniStave.this.notate.getY() + bounds.y;

      Component c = MiniStave.this;
      while( c.getParent() != null && c != MiniStave.this.notate )
        {
        x += c.getX();
        y += c.getY();
        c = c.getParent();
  ...
  protected void paintComponent(Graphics g)
    ...
    // It seems like it would be better to do this some other way.
...
*/
    ...
    xCoordinate = leftMargin + clefWidth + keySigWidth + timeSigWidth;
...
//            if(selectionControls.mouseOver == false)
//                selectionControls.setVisible(false);
//        } else {
//            selectionControls.setLocation(mouseOverSelection.x - 5, mouseOverSelection.y - 25);
...
//        }
    ...
    selectionRHandle.height = lineSpacing;
      ...
      {
//            if(getActionHandler().activeHandles()) {
//                g.setColor(selectionHandleColor);
//                g.translate(first.x, first.y);
//                g.fillPolygon(selectionHandle[0]);
//                g.translate(0, lineSpacing);
//                g.fillPolygon(selectionHandle[1]);
//                g.translate(last.x + last.width - first.x, last.y - first.y);
//                g.fillPolygon(selectionHandle[3]);
//                g.translate(0, -lineSpacing);
//                g.fillPolygon(selectionHandle[2]);
//                g.translate(-last.x - last.width, -last.y);
//
//                g.setColor(selectionBorderColor);
//                g.translate(first.x, first.y);
//                g.drawPolygon(selectionHandle[0]);
//                g.translate(0, lineSpacing);
//                g.drawPolygon(selectionHandle[1]);
//                g.translate(last.x + last.width - first.x, last.y - first.y);
//                g.drawPolygon(selectionHandle[3]);
//                g.translate(0, -lineSpacing);
//                g.drawPolygon(selectionHandle[2]);
//                g.translate(-last.x - last.width, -last.y);
//            }

//            if(marqueeSelection.width > 0 && marqueeSelection.height > 0) {
//                g.setColor(marqueeBorderColor);
//                g.drawRect(marqueeSelection.x, marqueeSelection.y, marqueeSelection.width, marqueeSelection.height);
//            }
      }
    }
    ...
    for( int i = 0; i < cstrLines.length; i++ )
      ...
      if( chordProg.getSectionInfo().sectionAtSlot(i) )
        ...
        g.drawString("Style: " +
                ...
                xCoordinate - 25,
                headSpace + (staveLine * lineSpacing) - 40);
        }

      // Check to see if the proper resolution is set for a beat
      if( i % beatValue == 0 )
        {
        int sdivs = checkBeatResolution(i / beatValue);

        // Half-note triplets
        if( i % measureLength == 0 && orignote != null && orignote.getRhythmValue() == 160 )
          {
          tupletStart = xCoordinate - 10;
          tupletStartIndex = i;
          tupletValue = 3;
          tupletEndIndex = i + measureLength - 1;
          }

        // Quarter-note triplets
        if( i % beatValue == 0 && orignote != null && orignote.getRhythmValue() == 80 )
          {
          tupletStart = xCoordinate - 10;
          tupletStartIndex = i;
          tupletValue = 3;
          tupletEndIndex = i + 239;
          }
        // Eighth-note triplets
        else if( (sdivs % 3 == 0 || sdivs % 5 == 0) && tupletEndIndex < i )
          {
          tupletStart = xCoordinate - 10;
          tupletStartIndex = i;
          tupletValue = sdivs;
          tupletEndIndex = checkForTupletEnd(i, tupletValue, sdivs, part);
          }
      ...
      if( cstrLines[i] != null )
        ...
        // Give selection lines priority over cut-from pasting.
        ...
        if( note != null && i < noteColors.length )
          {
          if( i >= selectionStart && i <= selectionEnd && (note.firstTied() || !note.isTied()) && note.getPitch() != Note.REST )
            {
            drawNote(note, true, i, g, noteColors[i], part);
            boxTies = true;
            }
            ...
            {
            drawNote(note, false, i, g, noteColors[i], part);
            boxTies = false;
            }

          lastNote = note;
          if( yCoordinate < highestYPos )
            {
            highestYPos = yCoordinate;
            }
      ...
      if( (i + 1) % beatValue == 0 )
        ...
        if( tupletValue != OUT_OF_BOUNDS && tupletEndIndex == i )
          ...
          for( int j = 0; j <= tupletValue; ++j )
            ...
            if( part.getNote(index) != null &&
                    !part.getNote(index).isRest() &&
                    part.getNote(index - 1) != part.getNote(index) &&
                    part.getNote(index).getRhythmValue() < tupletBeatValue * tupletValue &&
                    part.getNote(index).getRhythmValue() % tupletBeatValue == 0 ||
                    tupletEndIndex - tupletStartIndex > beatValue )
              {
      ...
      if( (i + 1) % measureLength == 0 )
        ...
        if( showBarNums && (lineMeasureCount != measuresOnLine) )
          {
          g.drawString("" + (int)(totalMeasureCount + 1),
                  xCoordinate - 4,
      ...
      if( toNextLine == true && i != part.size() - 1 )
        ...
        // the end of the line
        if( lastNote != null && lastNote.isTied() && (totalMeasureCount * measureLength) < part.size() && part.getNote(totalMeasureCount * measureLength).isTied() && !part.getNote(totalMeasureCount * measureLength).firstTied() && isNote )
          {
          drawTie(startOfTie, STAVE_WIDTH + 10, g);
   ...
   * already found. Edited from original code, which had drawNote and
   * drawNote2. Handles ties by reading whether a note is tied from a Note
   ...
   */
  private void drawNote(Note note, boolean boxed, int i, Graphics g,
                         int noteColor, MelodyPart part)
    {
    drawNote(note, boxed, i, g, noteColor, OUT_OF_BOUNDS, part);
    }

  private void drawNote(Note note, boolean boxed, int i, Graphics g,
                         int noteColor,
                         int forcedType, MelodyPart part)
    {
    ...
    switch( type )
      ...
      case TREBLE:
        unitImage = chooseImage(pitch, rhythmValue, noteColor, 71, 0, 71);
        break;

      case BASS:
        unitImage = chooseImage(pitch, rhythmValue, noteColor, 50, 0, 50);
        break;

      case GRAND:
        unitImage = chooseImage(pitch, rhythmValue, noteColor, 71, 60, 50);
        break;
      }
...
//
//            g.setColor(boxColor);
//            g.fillRect(drawWidth - 2, yCoordinate - 2, unitImage.getWidth(this)
//                    + 2, unitImage.getHeight(this) + 2);
//            g.setColor(Color.black);
      }
    ...
    if( dottedNote )
      ...
      for( int l = 0; l < lineNotes.length; l++ )
        {
        if( lineNotes[l] + 12 == pitch || lineNotes[l] + 36 == pitch || lineNotes[l] + 60 == pitch || lineNotes[l] + 84 == pitch || lineNotes[l] + 108 == pitch || pitch == REST )
          {
  ...
  public int[] collectNoteColors(MelodyPart part, Graphics g)
    {
    int[] color = new int[cstrLines.length];
    ...
    for( int i = 0; i < cstrLines.length; i++ )
      ...
      if( curNote != null )
        {
        Note origNote = this.getOrigPart().getNote(i);
  ...
  public int determineColor(Note note, Note pitchDeterminer, int i, Graphics g,
    ...
    int prevIndex = this.getOrigPart().getPrevIndex(i);
    Note prevNote = this.getOrigPart().getNote(prevIndex);
  ...
  private int drawTimeSig(Graphics g)
    {
    Image[] numbers = {one, two, three, four, five, six, seven, eight,
                       nine, zero
    };
    ...
    int tempTimeSigWidth = 30;
    return tempTimeSigWidth;
    }
  ...
  private void drawBeatBracket(int subdivs, int start, int end, int staveLine,
    ...
    g.drawImage(beatBracket,
            // stretch from x1, y1
            start, bracketHeight,
            // stretch to x2, y2
            end, bracketHeight + beatBracket.getHeight(this),
            // initial image x1, y1
            0, 0,
            // initial image x2, y2
            beatBracket.getWidth(this), beatBracket.getHeight(this),
            this);
   ...
   * @param g             the panel to draw onto
   */
  private void drawTupletBracket(int n, int start, int end, int staveLine,
                                  Graphics g)
    {
    ...
    if( end < start )
      ...
      g.drawImage(tupletBracket,
              // stretch from x1, y1
              start, bracketHeight,
              // stretch to x2, y2
              topEnd, bracketHeight + tupletBracket.getHeight(this),
              // initial image x1, y1
              0, 0,
              // initial image x2, y2
              tupletBracket.getWidth(this), tupletBracket.getHeight(this),
              this);

      g.drawString("" + n, start + ((topEnd - start) / 2) - 4, bracketHeight + 5);
      staveLine++;
      start = leftMargin - 15;
      ...
      g.drawImage(tupletBracket,
              ...
              start, bracketHeight,
              // stretch to x2, y2
              end, bracketHeight + tupletBracket.getHeight(this),
              // initial image x1, y1
              20, 0,
              // initial image x2, y2
              tupletBracket.getWidth(this), tupletBracket.getHeight(this),
  ...
  private void drawLegerLine(int pitch, int x, int staveLine, Graphics g)
    ...
    // NOTE: These values were left over from jMusic, I just moved them
    // into arrays and ignored the ugliness
    int[] treblePitches = {61, 58, 54, 51, 48, 81, 84, 88, 91, 95};
    int[] bassPitches = {40, 37, 34, 30, 26, 60, 64, 67, 71, 74};
    int[] grandPitches = {40, 37, 34, 30, 27, 81, 84, 88, 91, 95};

    // Arrays of the y-axis offsets corresponding to each stave
    int[] trebleOffset = {40, 48, 56, 64, 72, -8, -16, -24, -32, -40};
    int[] bassOffset = {40, 48, 56, 64, 72, -8, -16, -24, -32, -40};
    int[] grandOffset = {88, 96, 104, 112, 120, -8, -16, -24, -32, -40};
   ...
   * @param g                 the panel to draw onto
   */
  private void drawTie(int startOfTie, int endOfTie, Graphics g)
    {
   ...
   * @see #drawNote(Note, boolean, Graphics)
   */
  private Image chooseImage(int pitch, int rhythmValue, int color,
                             int upPitch1, int downPitch, int upPitch2)
    {
  ...
  void unselectAll()
    ...
    setSelectionStart(OUT_OF_BOUNDS);
    setSelectionEnd(OUT_OF_BOUNDS);
    }

  void smartTransposeMelody()
    {
  ...
  public void shiftPitch(int startIndex, int endIndex, boolean up,
    ...
    getActionHandler().redoAdvice(startIndex);
    repaint();
    }

  private void resolvePitch(int startIndex, int endIndex)
    {
   ...
   * @param endIndex         ending index of the selection of notes
   */
  public void playSelection(boolean playToEnd, int loopCount)
    {
    ...
    MelodyPart part = getOrigPart();

    int startIndex = getSelectionStart();
    int stopIndex = playToEnd ? part.getSize() - 1 : getSelectionEnd();

    //System.out.println("Play Selection from " + startIndex + " to " + stopIndex);

    playingSlot = startIndex;

    ChordPart chords = getChordProg();

    Score score = new Score();

    score.setTempo(notate.score.getTempo());

    MelodyPart extractedMelody = part.extract(startIndex, stopIndex);

    extractedMelody.addUnit(new Rest(EIGHTH));        // to let playback finish

    extractedMelody.setInstrument(part.getInstrument());

    score.addPart(extractedMelody);

    ChordPart extractedChords = chords.extract(startIndex, stopIndex);

    extractedChords.setInstrument(chords.getInstrument());

    // The following is needed to prevent playback from getting cut off abruptly.
    // It does seem a little strange, but make sure you have a better way before removing.

    extractedChords.addUnit(new Chord(NOCHORD, beatValue));

    score.setChordProg(extractedChords);

    notate.initCurrentPlaybackTab(startIndex);

    notate.cm.execute(new PlayScoreCommand(score, 0, true,
            notate.getMidiSynth(), notate, loopCount, notate.getTransposition()));
    notate.setShowPlayLine(true);
    repaint();
    }
        ...
        case BOTH:
         ...
         Note prevNote = (Note)melody.getUnit(startIndex);
          ...
          for( ; index <= stopIndex; index++ )
            ...
            if( unit != null )
              {
              prevNote.setRhythmValue(index - prevIndex);
