 ...
 */
public class MiniStaveActionHandler
        implements Constants, MouseListener,
                   MouseMotionListener, KeyListener
  {
  
  /**
   * bias the note click position in the y direction
   * See also verticalAdjustment.
   */
  
  static public int yNoteClickBias = -2;
   ...
   */
  public static final int pitchFromSpacing[] = {c4, d4, e4, f4, g4, a4, b4,
                                                   c5
  };
   ...
   */
  private MiniStave stave;

  /**
   * The notation window for the Stave
   */
  private MiniNotate notate;
...
//    MelodyContour[] contours = new MelodyContour[2];
   ...
   */
  MiniStaveActionHandler(MiniStave stave, MiniNotate notate)
    {
    this.stave = stave;
    this.notate = notate;
    ...
    Icon icon =
            new javax.swing.ImageIcon(getClass().getResource("graphics/toolbar/pencilCursor.png"));
    BufferedImage bi = new BufferedImage(icon.getIconWidth(),
            icon.getIconHeight(), BufferedImage.TYPE_INT_ARGB);
    icon.paintIcon(null, bi.getGraphics(), 0, 0);
    penCursor = toolkit.createCustomCursor(bi, new Point(0,
            icon.getIconHeight() - 1), "Pencil");
    }
  ...
  public void maybeSetCursor(MouseEvent e)
    ...
    if( stave.getShowSheetTitle() )
      {
      if( stave.sheetTitleEditor.checkEvent(e) )
        {
        setCursor();
        }
      }
  ...
  public void mouseMoved(MouseEvent e)
    ...
    if( inNoteArea(e) && stave.getShowMeasureCL() && !stave.getShowAllCL() )
      {
    ...
    if( !drawing )
      ...
      if( mouseOverLHandle() && !stave.nothingSelected() )
        ...
        {
        setCursor();     // TODO: make this get called less
        }
      }
  ...
  private int yPosToKeyPitch(int y, int currentLine)
    ...
    // I'm not sure whether yNoteClickBias is related to verticalAdjustment.
    
    int pitch = yPosToPitch(y + yNoteClickBias, currentLine);
  ...
  private int addNote(int x, int y, Chord chord, boolean shiftDown)
    ...
    Polylist scaleTones = form.getFirstScaleTones(chord.getRoot());
    Polylist chordTones = form.getSpell(chord.getRoot());
    Polylist colorTones = form.getColor(chord.getRoot());

    /* So far, the list of accpetable pitches to draw.
     * We want to disregard this in a moment if we're going to force
     * an approach tone.
     */
    Polylist l, m = new Polylist();
    if( drawScaleTones )
      {
  ...
  public void mousePressed(MouseEvent e)
    ...
    if( !selectingGroup && inNoteArea(e) && (e.getModifiersEx() & MouseEvent.BUTTON1_DOWN_MASK) != 0 && searchForBracket(e) == OUT_OF_BOUNDS )
      {
      ...
      drawing = false; // (notate.getMode() == Notate.Mode.DRAWING);
    ...
    if( (e.getModifiersEx() & MouseEvent.BUTTON1_DOWN_MASK) != 0 && inNoteArea(e) && !drawing )
      ...
      // check to see if a construction line was clicked on
        ...
        // or if the user has pressed within a selection of notes
                ...
                && selectedIndex <= stave.getSelectionEnd() 
                /* && stave.getSelectionStart() != stave.getSelectionEnd() */ ) // with this clause in, cannot drag single notes laterally
          {
          Trace.log(2,
        ...
        if( !stave.getChordProg().getCurrentChord(selectedIndex).getName().equals("NC") )
          {
    ...
    else if( (e.getModifiersEx() & MouseEvent.BUTTON3_DOWN_MASK) != 0 && inNoteArea(e) )
      {
  ...
  public void mouseDragged(MouseEvent e)
    ...
    if( selectingGroup && !timeSelected && !keySelected // with selectingGroup out, does not help dragging single notes
      ...
      stave.setCursor(new Cursor(Cursor.CROSSHAIR_CURSOR));
      ...
      if( e.getX() > startDragX )
        ...
        if( e.getY() > startDragY )
          ...
          stave.getGraphics().drawRect(startDragX, startDragY,
                  e.getX() - startDragX, e.getY() - startDragY);
          }
        else
          {
          stave.getGraphics().drawRect(startDragX, e.getY(),
                  e.getX() - startDragX, startDragY - e.getY());
          }
        }
      else
        {
        if( e.getY() > startDragY )
          ...
          stave.getGraphics().drawRect(e.getX(), startDragY,
                  startDragX - e.getX(), e.getY() - startDragY);
          }
        else
          {
      ...
      try
        ...
        if( (selectedIndex != lastIndexDrawn || (lastDrawnNote != null && lastDrawnNote.getPitch() != newPitch)) && selectedIndex != OUT_OF_BOUNDS )
          ...
          if( firedIndices.size() == 0 ||
                  (selectedIndex != lastIndexDrawn && selectedIndex != OUT_OF_BOUNDS && selectedIndex != firedIndices.get(firedIndices.size() - 1)) )
            {
...
// if a beat bracket was clicked on
    if( selectedBeat != OUT_OF_BOUNDS && !drawing )
      ...
      // dragging the selections's pitch
       ...
       if( e.getY() > lastDragY + 4 )
       {
       lastDragY = e.getY();
       stave.transposeMelodyDownSemitone();
       }
       else if( e.getY() < lastDragY - 4 )
       {
       lastDragY = e.getY();
    ...
    // dragging a group of note's position
      ...
      if( pasteIndex < stave.getOrigPart().size() )
        ...
        notate.cm.execute(new DragSetCommand(stave.getOrigPart(),
                ...
                draggingGroupOrigSelectionEnd, pasteIndex));
    ...
    if( !ImproVisor.getShowAdvice() )
      ...
      if( currentChord == null || currentChord.getName().equals(NOCHORD) )
        ...
        ErrorLog.log(ErrorLog.COMMENT,
                "To get advice, there must be a chord in effect.");
        return;
        }
      ...
      if( ((lastAdviceChord == null) ||
              (currentChord != null && !(currentChord.getName().equals(lastAdviceChord.getName())))) || ((lastAdviceNext == null) ||
              (currentNext != null && !(currentNext.getName().equals(lastAdviceNext.getName())))) || ((lastAdviceNote == null) ||
              (currentNote != null /* && !(lastAdviceNote.equals(currentNote) ) */)) )
        {

        Trace.log(2, "redoing Advice");
      ...
      if( e.isMetaDown() )
        ...
        switch( e.getKeyCode() )
          ...
          case KeyEvent.VK_ENTER:
            stave.playSelection(true, notate.getLoopCount());
            return;
          case KeyEvent.VK_Z:
            undo();
            return;
  ...
  private int getCurrentLine(int y)
    ...
    for( int i = 0; i <= stave.staveLine; i++ )
      {
      if( y > (i * stave.lineSpacing) + stave.headSpace - stave.lineOffset )
        {
        currLine = i;
        }
      }
  ...
  private int searchForCstrLine(int x, int y)
    ...
    for( int i = 0; i < stave.cstrLines.length; i++ )
      ...
      if( stave.cstrLines[i] != null )
        ...
        tempY =
                stave.cstrLines[i].getY() - (stave.numPitchLines * stave.staveSpaceHeight) / 2;

        // if it is within a small boundary in the x and y direction
        if( x > tempX - (stave.cstrLines[i].getSpacing() / 2) + 2 && x < tempX + (stave.cstrLines[i].getSpacing() / 2) - 2 && y > tempY - stave.lineOffset && y < tempY - stave.lineOffset + stave.lineSpacing )
          {
